                            ...
                            const unsigned char *dgst, int dlen)
{
    ...
    do {
        ...
        OSSL_DEBUG_BN((16,X,&xptr,r,&yptr,NULL),"X 0x%s r 0x%s",xptr,yptr);
    } while (BN_is_zero(r));
    ...
    OSSL_DEBUG_BN((16,k,&xptr,r,&yptr,NULL),"k 0x%s r 0x%s",xptr,yptr);
                                      ...
                                      EC_KEY *eckey)
{
    ...
    do {
        if (in_kinv == NULL || in_r == NULL) {
            ...
            ckinv = kinv;
            OSSL_DEBUG_BN((16,ckinv,&sptr,ret->r,&rptr,NULL),"ckinv 0x%s r 0x%s",sptr,rptr);
        } else {
            ckinv = in_kinv;
        ...
        //OSSL_DEBUG_BN((16,ret->r,&xptr,s,&yptr,NULL),"ret->r 0x%s s 0x%s",xptr,yptr);
        if (!bn_mul_mont_fixed_top(s, s, priv_key, group->mont_data, ctx)) {
            ERR_raise(ERR_LIB_EC, ERR_R_BN_LIB);
            goto err;
        }
        //OSSL_DEBUG_BN((16,priv_key,&xptr,s,&yptr,NULL),"priv_key 0x%s s 0x%s",xptr,yptr);
        /*
        if (!bn_to_mont_fixed_top(s, ret->r, group->mont_data, ctx)
        ...
        OSSL_DEBUG_BN((16,s,&sptr,NULL),"s 0x%s",sptr);
        /*
